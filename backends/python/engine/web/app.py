...
def run_web(host: str = "127.0.0.1", port: int = 8788) -> None:  # noqa: C901 — wires every page
    ...
    # NiceGUI's own session storage stays a file in the data dir — it is
    ...
    if not reachable:
        raise RuntimeError(f"Postgres unreachable — syncplex cannot serve without it: {detail}")
    ...
    def _requests(user: User) -> RequestStore:
        ...
        carries the session token that RLS reads — there is no process-wide
            ...
            with ui.dialog() as dialog, ui.card().classes("w-full max-w-md"):
                ...
                def _request_actions() -> None:
                    ...
                    if pending is not None:
                        ui.label("⏳ requested by you — waiting for admin approval").classes("req-pending")
